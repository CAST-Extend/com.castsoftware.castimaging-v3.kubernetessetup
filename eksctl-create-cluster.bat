...
eksctl create cluster --name %CLUSTER_NAME% --region %AWS_DEFAULT_REGION% --nodegroup-name %CLUSTER_NAME%-ng --nodes-min 2 --nodes-max 4 --node-type %NODE_TYPE% --nodes 2 --node-volume-size 100 --ssh-access  --with-oidc --zones %AWS_DEFAULT_REGION%a,%AWS_DEFAULT_REGION%b --node-zones %AWS_DEFAULT_REGION%b
